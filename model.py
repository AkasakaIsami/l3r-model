...
class StatementClassifierForSingleEdge(nn.Module):
    """
    语句分类器 但是单边版本
    """
    ...
    def __init__(self, embedding_dim=128, encode_dim=128, hidden_dim=32, encoder_num_layers=2, classifier_num_layers=3,
        ...
        self.encoder = StatementEncoder(self.embedding_dim, self.hidden_dim, self.encode_dim, self.dropout, self.gpu)
        self.conv_0 = Sequential('x, edge_index', [
            (GATConv(self.encode_dim, self.hidden_dim, heads=3, dropout=self.dropout),
             'x, edge_index -> x'),
            nn.Tanh(),
            BatchNorm(self.hidden_dim)
        ])

        self.conv_2 = Sequential('x, edge_index', [
            (GATConv(self.hidden_dim * 3, self.hidden_dim, heads=1, dropout=self.dropout),
             'x, edge_index -> x'),
            nn.Tanh(),
            BatchNorm(self.hidden_dim)
        ])
